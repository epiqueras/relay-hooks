import { __internal, createOperationDescriptor, getRequest, createRequestDescriptor, createReaderSelector, ConnectionInterface, isPromise, Observable, getVariablesFromFragment, getDataIDsFromFragment, getFragment, getSelector, commitMutation, requestSubscription } from 'relay-runtime';
...
import React, { useState as useState$1, useRef, useEffect, useMemo, useCallback as useCallback$1 } from 'react';
...
function getNewSelector(request, s, variables) {
    ...
    var selector = createReaderSelector(s.node, s.dataID, variables, requestDescriptor);
    return selector;
}
function _getConnectionData(_a, props, connectionConfig) {
    ...
    var _b = ConnectionInterface.get(), EDGES = _b.EDGES, PAGE_INFO = _b.PAGE_INFO, HAS_NEXT_PAGE = _b.HAS_NEXT_PAGE, HAS_PREV_PAGE = _b.HAS_PREV_PAGE, END_CURSOR = _b.END_CURSOR, START_CURSOR = _b.START_CURSOR;
...
var useQueryFetcher = function (query) {
    var _a = useState$1(null), forceUpdate = _a[1];
    var ref = useRef();
...
function useMemoOperationDescriptor(gqlQuery, variables) {
    ...
    return useMemo(function () { return createOperation(gqlQuery, memoVariables); }, [gqlQuery, memoVariables]);
}
...
var fetchQuery$1 = __internal.fetchQuery;
...
var FragmentResolver = /** @class */ (function () {
    function FragmentResolver(forceUpdate) {
        ...
        this.loadMore = function (connectionConfig, pageSize, observerOrCallback, options) {
            ...
            var connectionData = _getConnectionData(_this.paginationData, _this.getData(), connectionConfig);
            if (!connectionData) {
                Observable.create(function (sink) { return sink.complete(); }).subscribe(observer);
                return null;
            }
    ...
    FragmentResolver.prototype.changedFragmentRef = function (fragmentRef) {
        if (this._fragmentRef !== fragmentRef) {
            var prevIDs = getDataIDsFromFragment(this._fragment, this._fragmentRef);
...
environment) {
    ...
    var mutate = useCallback(function (config) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            commitMutation(resolvedEnvironment, __assign(__assign({}, mergedConfig), { mutation: mutation, variables: mergedConfig.variables, onCompleted: function (response, errors) {
                    if (errors) {
...
function useRefetchable(fragmentInput, fragmentRef) {
    ...
    var refetchNode = useMemo(function () {
    ...
    var refetchable = useCallback$1(function (refetchVariables, options) {
        if (options === void 0) { options = {}; }
...
var RelayEnvironmentProvider = function (props) {
    var context = React.useMemo(function () { return ({ environment: props.environment }); }, [props.environment]);
    return (React.createElement(ReactRelayContext.Provider, { value: context }, props.children));
};

export { FORWARD, NETWORK_ONLY, ReactRelayContext, RelayEnvironmentProvider, STORE_ONLY, STORE_OR_NETWORK, STORE_THEN_NETWORK, loadLazyQuery, loadQuery, useFragment, useLazyLoadQuery, useMutation, useOssFragment, usePagination, usePreloadedQuery, useQuery, useQueryFetcher, useRefetch, useRefetchable, useRelayEnvironment, useSubscription };
